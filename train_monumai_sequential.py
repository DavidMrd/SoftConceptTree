"""
Train InceptionV3 Network using the MonuMAI-200-2011 dataset
"""
...
#from MonuMAI import probe, tti, gen_cub_synthetic, hyperopt
#from MonuMAI.dataset import load_data, find_class_imbalance
...
from MonuMAI.models import ModelXtoCY, ModelXtoChat_ChatToY, ModelXtoY, ModelXtoC, ModelXtoCtoY_soft_decision_tree, ModelOracleCtoY,ModelOracleSoftTree, ModelXtoCtoY, ModelXtoChat_ChatToY_soft_decision_tree
from MonuMAI.template_model import SDT, End2EndModel_ResnetTree
...
def run_epoch_simple(model, optimizer, loader, loss_meter, acc_meter, criterion, args, is_training):
    ...
    for batch_idx,  (data, target) in enumerate(loader):
        ...
        batch_size = data.size()[0]
            #print("Input dim is "+ str(model.input_dim))
        if(is_training):    
            outputs, penalty = model.forward( data,is_training_data=True)
            ...
            #         " Correct: {:03d}/{:03d}"
            #     )
            #     print(msg.format( batch_idx, loss, correct, batch_size))
        else:
            outputs= model.forward( data)
            loss = criterion(outputs, target.view(-1))        
            output = F.softmax(outputs, dim=1)
            pred = output.data.max(1)[1]
            correct_i = pred.eq(target.view(-1).data).sum()
            correct += correct_i
            acc = accuracy(output, target, topk=(1,))
            acc_meter.update(float(acc[0]),data.size(0))

        loss_meter.update(loss,len(data))
...
def run_epoch(model, optimizer, loader, loss_meter, acc_meter, criterion, attr_criterion, args, is_training):
    ...
    for _, data in enumerate(loader):
        if attr_criterion is None:
            ...
            #     attr_labels = torch.stack(attr_labels)#.t() #<-ESTO es una diff!! por que??                   #.float() #N x 312
            # else:
        ...
        if is_training and args.use_aux:
            ...
            #loss_main = 0.0
            if( isinstance(model, End2EndModel_ResnetTree) and is_training):
                #print("Input dim is "+ str(model.input_dim))
                attr_outputs,outputs, penalty = model.forward( inputs_var,is_training_data=True)
                #loss_main = penalty + loss_main
                
                #loss = criterion(outputs, labels_var)
                
            else:    
                attr_outputs, outputs = model.forward(inputs_var)
            #print(len(outputs))
            #print(outputs.shape)
            losses = []
            out_start = 0
            if not args.bottleneck:
                loss_main = criterion(outputs, labels_var)
                if is_training:
                    loss_main= loss_main + penalty          
                losses.append(loss_main)
                #out_start = 1
            if attr_criterion is not None and args.attr_loss_weight > 0: #X -> A, cotraining, end2end
...
def train(model, args):
    if not args.use_inception and args.use_aux:        
        raise argparse.ArgumentError(None,"-use_aux only compatible with -use_inception")
    ...
    if args.use_attr and not args.no_img and args.weighted_loss:
        ...
        train_data_path = 'MonuMAI/train_onehot_seed'+str(args.seed)+'.csv'#os.path.join(BASE_DIR, args.data_dir, 'train.pkl')
    ...
    train_dataset, val_dataset, test_dataset  = get_dataset(train_file='MonuMAI/train_onehot_seed'+str(args.seed)+'.csv', val_file='MonuMAI/val_onehot_seed'+str(args.seed)+'.csv', test_file = 'MonuMAI/test_onehot_seed'+str(args.seed)+'.csv' , use_attr = args.use_attr, no_img = args.no_img, use_inception = args.use_inception)
...
def train_X_to_C(args):
    model = ModelXtoC(pretrained=args.pretrained, freeze=args.freeze, num_classes=N_CLASSES, use_aux=args.use_aux,
                      n_attributes=args.n_attributes, expand_dim=args.expand_dim, three_class=args.three_class,use_inception=args.use_inception)
    train(model, args)
...
def train_oracle_C_to_y_and_test_on_Chat_soft_decision_tree(args):
    print('train_oracle_C_to_y_soft \n')
    model = ModelOracleSoftTree(n_class_attr=args.n_class_attr, n_attributes=args.n_attributes,
                            num_classes=N_CLASSES, expand_dim=args.expand_dim)
    train(model, args)
...
def train_Chat_to_y_and_test_on_Chat(args):
    model = ModelXtoChat_ChatToY(n_class_attr=args.n_class_attr, n_attributes=args.n_attributes,
                                 num_classes=N_CLASSES, expand_dim=args.expand_dim)
    train(model, args)
def train_Chat_to_y_and_test_on_Chat_soft_decision_tree(args):
    model = ModelXtoChat_ChatToY_soft_decision_tree(n_class_attr=args.n_class_attr, n_attributes=args.n_attributes,
                                 num_classes=N_CLASSES, expand_dim=args.expand_dim)
    train(model, args)
def train_X_to_C_to_y(args):
    ...
    train(model, args)
def train_X_to_C_to_y_soft_decision_tree(args):
    model = ModelXtoCtoY_soft_decision_tree(n_class_attr=args.n_class_attr, pretrained=args.pretrained, freeze=args.freeze,
                         num_classes=N_CLASSES, use_aux=args.use_aux, n_attributes=args.n_attributes,
                         expand_dim=args.expand_dim, use_relu=args.use_relu, use_sigmoid=args.use_sigmoid)
    train(model, args)
...
def parse_arguments(experiment):
    ...
    parser.add_argument('exp', type=str,
                        choices=['Concept_XtoC', 'Independent_CtoY','Independent_CtoY_SoftDecisionTree', 'Sequential_CtoY','Sequential_CtoY_SoftDecisionTree',
                                 'Standard', 'Multitask', 'Joint', 'Joint_SoftDecisionTree', 'Probe',
    ...
    if experiment == 'Probe':
        ...
        parser.add_argument('-use_inception', action='store_true', help='whether to use inception or resnet')
